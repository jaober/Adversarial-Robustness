"""
Performs hyperparameter tuning for retraining a pre-trained source model's fully connected layers
on a new target dataset. Parameters to be fine-tuned are learnig rate and regularization strength.
The main function run_hyperparameter_search returns the validation accuracy of all trained models and saves the model
achieving the best accuracy on the validation dataset to a file.
"""
...
import seml
from LCP_utils import AggMo, NpyDataset, write_json, copy_file
...
def train(data_dir, model_path_layers_unfreeze, num_classes, log_step, num_epochs, regularization, batch_size,
          num_workers, learning_rate, patience, early_stopping, loss_func,
          sigma, max_nr_images, TL_label_map, LC_margin, WN_initialization, data_set):
    ...
    test_acc, test_loss = model_eval('Test', criterion, device, model, test_loader, fc_layers,
                                       step)
    naming_list = [nr_unfreeze, 200, learning_rate, regularization, num_classes]

    return best_train_acc, best_train_acc_loss, best_val_acc, best_val_acc_loss, test_acc, test_loss, early_stop, \
           naming_list, model
        ...
        for i, (images, targets) in enumerate(train_loader):
            if i == 0:
                ...
                for l in range(len(layer_sizes)):
                    ...
                    print(model.fc[2 * l].weight_v.data.shape, y.shape,
                          torch.norm(model.fc[2 * l].weight_v.data, dim=1).T.shape)
...
def print_training_info(num_epochs, batch_accuracy, epoch, i, iteration_time, loss, step, total_step):
...
def run_hyperparameter_search(param_grid, data_dir, model_path_layers_unfreeze, num_classes, log_step,
                              num_epochs, batch_size, num_workers, patience, early_stopping, loss_func,
                                                   sigma, max_nr_images, TL_label_map, LC_margin,
                              WN_initialization, data_set):

    best_model = None
    best_naming_list = []
    best_val_acc = 0
    params_result_dict = {'best_train_acc': 0, 'best_val_acc': 0, 'test_acc': 0, 'early_stop': 0,}
    ...
    for params in ParameterGrid(param_grid):
        ...
        train_acc, _, val_acc, _, test_acc, _, early_stop, naming_list, model = train(data_dir, model_path_layers_unfreeze,
                                                      num_classes, log_step, num_epochs, regularization,
                                                      batch_size, num_workers, learning_rate, patience,
                                                      early_stopping, loss_func,
                                                      sigma, max_nr_images, TL_label_map, LC_margin,
                                                                                      WN_initialization, data_set)

        params_result_dict[f'lr {learning_rate}, reg {regularization}'] = val_acc

        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_model = model
            best_naming_list = naming_list
            params_result_dict['best_train_acc'] = train_acc
            params_result_dict['best_val_acc'] = val_acc
            params_result_dict['test_acc'] = test_acc
            params_result_dict['early_stop'] = early_stop

    run_id = save_model_checkpoint('models/', 100, best_model, best_val_acc, best_naming_list)

    params_result_dict['run_id'] = run_id
    return params_result_dict



@ex.automain
def run(param_grid, data_dir, model_path_layers_unfreeze, num_classes, log_step, num_epochs, batch_size, num_workers,
        patience, early_stopping, loss_func, sigma, max_nr_images, TL_label_map, LC_margin,
        WN_initialization, data_set=False):
    logging.info(f'param_grid: {param_grid}, data_set: {data_set}')


    params_result_dict = run_hyperparameter_search(param_grid, data_dir, model_path_layers_unfreeze, num_classes, log_step,
                              num_epochs, batch_size, num_workers, patience, early_stopping, loss_func,
                                                   sigma, max_nr_images, TL_label_map, LC_margin,
                                                   WN_initialization, data_set)
